...
void start_program(Program *programs, int num_programs, int cur) 
{
    ...
    else 
    {
        ...
        if (cur > 0 && programs[cur].fd_in >= 0) 
        {
            close(programs[cur].fd_in);
        }
        if (cur != num_programs - 1 && programs[cur].fd_out >= 0) 
        {
...
int wait_on_program(Program *prog)
{
    ...
    // TODO
    else if(waitpid(prog->pid, &exitStatus, 0) != prog->pid) {
...
void prepare_pipes(Program *programs, int num_programs){
    ...
    for(int i = 0; i < num_programs-1; i ++)        
    {
        if(pipe(pipefd)!=0)
        {
            die("pipe() failed prep.");
        }
        programs[i].fd_out = pipefd[1];
        programs[i + 1].fd_in = pipefd[0];
    }
}
