...
int one_round(THost *hosts, int m, node *p_arr[], int n_arr, int k, int T)
{
	...
	for(int i = 0; i < m; i++)
	{
		...
		switch(r)
		{
			case 0: if (hosts[i].y == k)
						{
                		hosts[i].y=((hosts[i].y + 1 + k) % (2 * k + 1))-k;
						}
					else {
                		hosts[i].y=((hosts[i].y + 1) % (2 * k + 1));
						}
					break;
            ...
            if (hosts[i].x==k)
            {
                hosts[i].x=((hosts[i].x + 1 + k) % (2 * k + 1))-k;
            }
            else
            {
                hosts[i].x=((hosts[i].x + 1) % (2 * k + 1));
            }
            break; 


			case 2: if (hosts[i].y == -k) 
						{
                		hosts[i].y=((hosts[i].y - 1 + k) % (2 * k + 1))+k+1;
						}
					else {
               			hosts[i].y=((hosts[i].y - 1) % (2 * k + 1));
						}
            ...
            if (hosts[i].x==-k)
            {
                hosts[i].x=((hosts[i].x - 1 + k) % (2 * k + 1))+k+1;
            }
            else
            {
                hosts[i].x=((hosts[i].x - 1) % (2 * k + 1));
