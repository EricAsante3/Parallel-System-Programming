...
void add_to_buffer(int item, int col, two_d_buffer *p)
{
	...
	p->buf[p->counts[col]][col] = item; //adding item to first free space in the column 
	p->counts[col]++;
	p->remain--;
...
void remove_from_buffer(int *a, int *b, int *c, two_d_buffer *p)
{
	...
	(*a) = p->buf[0][0]; //setting a, b, and c equal to columns 0, 1, and 2 in the first row
	(*b) = p->buf[0][1];
	(*c) = p->buf[0][2];

	for (int i = 0; i < (p->size - 1); i++) { //shifting positions/updating the index while also deleting first row
		p->buf[i][0] = p->buf[i + 1][0];
		p->buf[i][1] = p->buf[i + 1][1];
		p->buf[i][2] = p->buf[i + 1][2];
	}

	p->counts[0]--; //each column now has one less item so we gotta remove one from each counts
	p->counts[1]--;
	p->counts[2]--;
...
{   
	...
	while(!done)
	{
		...
		//fill in code below
		pthread_mutex_lock(&p->mutex);
		node *item = remove_first(&p->head, &p->tail); //FIFO order queue so we remove item first for production
		pthread_mutex_unlock(&p->mutex);

		if(item != NULL){
			int i = item->v;
			free(item); //deleting the node
			prepare(i);
			add_to_buffer(i, i, q);
			my_data->total++;
		}
        ...
        if (q->remain == 0) { //if this is true, that means there's nothing more to produce, kinda like a base case
            done = 1;
        }
...
int main(int argc, char *argv[])
{
	...
	//TODO
	//fill in code below
	total = 3 * n_consumer; //our total will always be 3 times the number of customers/consumers
	printf("total = %d\n", total);
